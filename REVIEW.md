...
1. since per-partition calibration drifts across geohash cells (slide 9), wouldn't it make sense to compute ID per-geohash instead of one global estimate? could be interesting as future work to see how much it varies across partitions.

1. would be interesting to compare OPQ vs turboquant. in theory OPQ should do better at 2-3 bits since it learns the rotation that minimizes quantization error instead of a random ones. (I did not read the turboquant paper yet!)
