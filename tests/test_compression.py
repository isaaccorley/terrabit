...
from terrabit.compression import compress_and_write
...
def _write_fake_dataset(root: Path, n_partitions: int = 2, n_rows: int = 200) -> None:
    ...
    for i in range(n_partitions):
        ...
        col = pa.FixedSizeListArray.from_arrays(
            pa.array(emb.ravel(), type=pa.float32()), list_size=dim,
        )
...
class TestPartitionedIO:
    ...
    def test_iter_embedding_batches(self, tmp_path: Path) -> None:
        ...
        total = 0
        for x, rb in iter_embedding_batches(str(tmp_path), batch_size=50):
            assert x.shape[1] == 64
...
class TestCompressAndWrite:
    ...
    def test_preserves_partition_structure(
        ...
        meta = compress_and_write(
            str(src), str(dst), method, n_components=8,  # type: ignore[arg-type]
        ...
        src_files = sorted(
            str(Path(f).relative_to(src)) for f in iter_parquet_files(str(src))
        )
        dst_files = sorted(
            str(Path(f).relative_to(dst)) for f in iter_parquet_files(str(dst))
        )
    ...
    def test_non_embedding_columns_preserved(self, tmp_path: Path) -> None:
        ...
        compress_and_write(
            str(src), str(dst), "ipca", n_components=4, batch_size=50,
        )
