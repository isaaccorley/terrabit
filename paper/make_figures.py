...
COMPRESSION = {
    "float16": 1.86, "fp8": 3.47, "int8": 3.45, "int4": 6.29,
    "int3": 7.94, "int2": 11.41, "binary": 16.53,
}
BITS = {
    "float16": 16, "fp8": 8, "int8": 8, "int4": 4,
    "int3": 3, "int2": 2, "binary": 1,
}
STORAGE_GIB = {
    "original": 182.88,
    "float16": 98.24, "fp8": 52.70, "int8": 52.96, "int4": 29.08,
    "int3": 23.02, "int2": 16.02, "binary": 11.07,
}
...
for ax, metric_key, title in [
    ...
    for x, y, m in zip(xs, ys, labels):
        ax.scatter([x], [y], color=COLORS[m], s=50, zorder=3,
                   edgecolors="white", linewidths=0.5, alpha=0.95)
        dx, dy = LABEL_OFFSETS[m]
        ax.annotate(
            m, (x, y), textcoords="offset points",
            xytext=(dx, dy),
            fontsize=6.5, ha="center", color=COLORS[m], fontweight="bold",
        )

    # Mark the int8 baseline
    ax.axhline(y=metrics["int8"][metric_key], color=ACCENT, linestyle="--",
...
bars = ax.barh(range(len(methods_bar)), sizes, color=colors_bar,
               edgecolor="white", linewidth=0.8, height=0.65, alpha=0.88)
...
ax.set_title("Corpus Storage Footprint (49.8M embeddings, d=1024)", fontsize=9,
             fontweight="bold")
ax.invert_yaxis()
...
for bar, size in zip(bars, sizes):
    ax.text(bar.get_width() + 1.5, bar.get_y() + bar.get_height() / 2,
            f"{size:.1f}", va="center", fontsize=7, color="#555")
...
for k in ks:
    ...
    ax.plot(compressions, vals, "o-", label=f"k={k}", markersize=4,
            linewidth=1.3, color=K_COLORS[k], alpha=0.9)

ax.set_xlabel("Compression Ratio (×)")
ax.set_ylabel("kNN Recall (Cosine)")
ax.set_title("Neighborhood Preservation Across k", fontsize=9,
             fontweight="bold")
...
bars = ax.bar(ests, vals, yerr=stds, color=bar_colors,
              edgecolor="white", linewidth=0.8, width=0.5, capsize=3,
              error_kw={"ecolor": "#999", "linewidth": 0.8}, alpha=0.88)
ax.axhline(y=1024, color=ACCENT, linestyle="--", alpha=0.5, linewidth=0.8,
           label="Ambient dim (1024)")
ax.set_ylabel("Intrinsic Dimension")
ax.set_title("ID Estimates (d=1024 embeddings)", fontsize=9,
             fontweight="bold")
